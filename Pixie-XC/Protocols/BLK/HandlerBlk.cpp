...
HandlerBlk::HandlerBlk(Protocol _protocol, socket_handle_t socketHandle, int _id)
{
    id = _id;
...
void HandlerBlk::handleBlkNormal()
{
...
void HandlerBlk::tunnelBlkProxy(BlkMessage& msg)
{
...
//    bool blk2 = socket_is_blocking(socket_fd);
    ...
    LOG(DEBUG) << "tunnel request connection upstreamSocket: " << upstreamRawSocket << " status: " << status << std::endl;
...
void HandlerBlk::normalBlkProxy(BlkMessage& msg)
{
...
void HandlerBlk::handleBlkLoopBack()
{
    BlkSocket myMessageSocket(socket_fd);
